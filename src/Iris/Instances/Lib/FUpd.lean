...
instance uPred_bi_fupd_plainly_no_lc {GF : BundledGFunctors} [INV : InvGS_gen false GF]
  ...
  fupd_plainly_mask_empty E P := by
    ...
    iintro H G
    have : Persistent iprop(■ P) := by sorry
    have : Plain iprop(◇ ■ P) := by sorry
    ihave #>HP : ◇ ■ P $$ [H G]
    · imod H $$ G with ⟨_, _, H⟩
      iassumption
    imodintro; imodintro
  ...
  fupd_plainly_sForall_2 E P := by
    simp only [fupd, uPred_fupd, le_upd_if, Bool.false_eq_true, ↓reduceIte]
    iintro H ⟨Hwsat, HE⟩
    have : Persistent (PROP := IProp GF) (iprop(■ sForall P)) := by sorry
    have : Plain (PROP := IProp GF) (iprop(◇ ■ sForall P)) := by sorry
...
theorem step_fupdN_soundness_no_lc [InvGpreS GF]
  ...
  have : Plain iprop(▷^[n.succ]P) := sorry
  have : Persistent iprop(P) := sorry
