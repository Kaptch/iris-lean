...
theorem persistently {Φ : Nat → A → PROP} {l : List A} :
  ...
  equiv_iff.mp <| BigOpL.hom bi_persistently_and_homomorphism Φ l
...
theorem zip_seq {Φ : A × Nat → PROP} {n : Nat} {l : List A} :
    ([∧list] ky ∈ l.zipIdx n, Φ ky) ≡
      [∧list] i ↦ x ∈ l, Φ (x, n + i) :=
      BigOpL.zipIdx (op := and) (unit := iprop(True)) Φ n l
...
theorem later {Φ : Nat → A → PROP} {l : List A} :
  ...
  equiv_iff.mp <| BigOpL.hom bi_later_and_homomorphism Φ l
