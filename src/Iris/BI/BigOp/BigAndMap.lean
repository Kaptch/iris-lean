...
theorem empty {Φ : K → V → PROP} :
    ...
    simp only [bigAndM, toList, toList_empty, bigOpL]
    exact .rfl
...
theorem singleton {Φ : K → V → PROP} {k : K} {v : V} :
  ...
  have hget : get? (∅ : M V) k = none := get?_empty k
  have hperm : (toList (insert (∅ : M V) k v)).Perm ((k, v) :: toList (∅ : M V)) :=
    LawfulFiniteMap.toList_insert hget
  simp only [toList, LawfulFiniteMap.toList_empty] at hperm
  simp only [bigAndM, PartialMap.singleton]
  have heq : bigOpL and (fun _ kv => Φ kv.1 kv.2) (toList (insert (∅ : M V) k v)) ≡
             bigOpL and (fun _ kv => Φ kv.1 kv.2) [(k, v)] :=
...
theorem insert {Φ : K → V → PROP} {m : M V} {k : K} {v : V}
    (h : get? m k = none) :
    ([∧map] k' ↦ x ∈ insert m k v, Φ k' x) ⊣⊢
      Φ k v ∧ [∧map] k' ↦ x ∈ m, Φ k' x := by
  simp only [bigAndM]
  have hperm_eq : bigOpL and (fun _ kv => Φ kv.1 kv.2) (toList (PartialMap.insert m k v)) ≡
                  bigOpL and (fun _ kv => Φ kv.1 kv.2) ((k, v) :: toList m) :=
    BigOpL.perm _ (LawfulFiniteMap.toList_insert h)
...
--     ([∧map] k' ↦ x ∈ PartialMap.insert m k v, Φ k' x) ⊣⊢
--       Φ k v ∧ [∧map] k' ↦ x ∈ delete m k, Φ k' x := by
--   -- have hmap_eq := LawfulPartialMap.insert_delete m k v
--   simp only [bigAndM]
--   rw [LawfulFiniteMap.toList_insert]
...
theorem delete' {Φ : K → V → PROP} {m : M V} {k : K} {v : V}
  ...
  simp only [bigAndM]
  have hperm := LawfulFiniteMap.toList_delete h
  have heq : bigOpL and (fun _ kv => Φ kv.1 kv.2) (toList m) ≡
             bigOpL and (fun _ kv => Φ kv.1 kv.2) ((k, v) :: toList (Std.delete m k)) :=
    BigOpL.perm _ hperm
  simp only [bigOpL] at heq
...
private theorem mono_list {Φ Ψ : K × V → PROP} {l : List (K × V)}
    (h : ∀ kv, kv ∈ l → Φ kv ⊢ Ψ kv) :
    bigOpL and (fun _ kv => Φ kv) l ⊢ bigOpL and (fun _ kv => Ψ kv) l := by
  induction l with
...
theorem mono {Φ Ψ : K → V → PROP} {m : M V}
  ...
  apply mono_list
  intro kv hmem
  have hkv : get? m kv.1 = some kv.2 := LawfulFiniteMap.toList_get.mp hmem
...
instance affine {Φ : K → V → PROP} {m : M V} [BIAffine PROP] :
  ...
  affine := by
    simp only [bigAndM, toList]
    induction (LawfulFiniteMap.toList (K := K) m) with
...
--     Φ k v ∧ ([∧map] k' ↦ x ∈ m, Φ k' x) ⊢ [∧map] k' ↦ x ∈ PartialMap.insert m k v, Φ k' x := by
--   cases hm : get? m k with
--   | none =>
--     exact (insert hm).2
--   | some y =>
--     have hdel := delete' (Φ := Φ) (m := m) hm
--     refine (and_mono_r (hdel.1.trans and_elim_r)).trans _
...
theorem and' {Φ Ψ : K → V → PROP} {m : M V} :
    ([∧map] k ↦ x ∈ m, Φ k x ∧ Ψ k x) ⊣⊢
      ([∧map] k ↦ x ∈ m, Φ k x) ∧ [∧map] k ↦ x ∈ m, Ψ k x := by
  simp only [bigAndM]
  exact equiv_iff.mp (BigOpL.op_distrib _ _ _)
...
theorem persistently {Φ : K → V → PROP} {m : M V} :
  ...
  simp only [bigAndM]
  exact equiv_iff.mp <| BigOpL.hom bi_persistently_and_homomorphism _ (toList m)
...
theorem fmap {Φ : K → V' → PROP} {m : M V} (f : V → V') :
  ...
  simp only [bigAndM, toList]
  refine equiv_iff.mp (BigOpL.perm _ (LawfulFiniteMap.toList_map (K := K))) |>.trans ?_
  induction (LawfulFiniteMap.toList (K := K) m) with
  | nil => exact .rfl
  | cons kv kvs ih =>
...
private theorem omap_list_aux {Φ : K → V → PROP} (f : V → Option V) (l : List (K × V)) :
    bigOpL and (fun _ kv => Φ kv.1 kv.2)
      (l.filterMap (fun kv => (f kv.2).map (kv.1, ·))) ⊣⊢
    bigOpL and (fun _ kv => match f kv.2 with | some y' => Φ kv.1 y' | none => iprop(True)) l := by
  induction l with
...
theorem omap {Φ : K → V → PROP} {m : M V} (f : V → Option V) (Hf : Function.Injective f) :
    ([∧map] k ↦ y ∈ PartialMap.filterMap (M := M) f m, Φ k y) ⊣⊢
      [∧map] k ↦ y ∈ m, match f y with | some y' => Φ k y' | none => iprop(True) := by
  simp only [bigAndM]
  exact equiv_iff.mp (BigOpL.perm _ (LawfulFiniteMap.toList_filterMap (K := K) Hf)) |>.trans
    (omap_list_aux f (toList m))

-- /-- Corresponds to `big_andM_union` in Rocq Iris. -/
-- theorem union {Φ : K → V → PROP} {m₁ m₂ : M V}
--     (hdisj : m₁ ##ₘ m₂) :
--     ([∧map] k ↦ y ∈ m₁ ∪ m₂, Φ k y) ⊣⊢
--       ([∧map] k ↦ y ∈ m₁, Φ k y) ∧ [∧map] k ↦ y ∈ m₂, Φ k y := by
--   simp only [bigAndM]
--   refine equiv_iff.mp (BigOpL.perm _ (_ m₁ m₂ hdisj)) |>.trans ?_
...
theorem intro {P : PROP} {Φ : K → V → PROP} {m : M V}
  ...
  simp only [bigAndM, toList]
  generalize hl : LawfulFiniteMap.toList (K := K) m = l
  induction l generalizing m with
  | nil => exact true_intro
  | cons kv kvs ih =>
    simp only [bigOpL]
    have hmem_kv : kv ∈ LawfulFiniteMap.toList m := hl ▸ List.mem_cons_self
    have hget_kv := LawfulFiniteMap.toList_get.mp hmem_kv
    refine and_intro (h kv.1 kv.2 hget_kv) ?_
    have htail : ∀ kv', kv' ∈ kvs → get? m kv'.1 = some kv'.2 := fun kv' hmem =>
      LawfulFiniteMap.toList_get.mp (hl ▸ List.mem_cons_of_mem _ hmem)
...
def mapForall (φ : K → V → Prop) (m : M V) : Prop :=
...
theorem pure_1 {φ : K → V → Prop} {m : M V} :
  ...
  suffices h : ∀ l : List (K × V),
      bigOpL and (fun _ (kv : K × V) => iprop(⌜φ kv.1 kv.2⌝)) l ⊢
        iprop(⌜∀ kv, kv ∈ l → φ kv.1 kv.2⌝) by
    refine (h (toList m)).trans <| pure_mono fun hlist k v hget => ?_
    have hmem : (k, v) ∈ toList m := LawfulFiniteMap.toList_get.mpr hget
...
theorem pure_2 {φ : K → V → Prop} {m : M V} :
  ...
  suffices h : ∀ l : List (K × V),
      iprop(⌜∀ kv, kv ∈ l → φ kv.1 kv.2⌝) ⊢
        bigOpL and (fun _ (kv : K × V) => iprop(⌜φ kv.1 kv.2⌝)) l by
    refine (pure_mono fun hmap kv hmem => ?_).trans (h (toList m))
    have hget : get? m kv.1 = some kv.2 := LawfulFiniteMap.toList_get.mp hmem
...
theorem later {Φ : K → V → PROP} {m : M V} :
  ...
  simp only [bigAndM]
  exact equiv_iff.mp <| BigOpL.hom bi_later_and_homomorphism _ (toList m)
...
private theorem filter_list_aux {Φ : K × V → PROP} (p : K × V → Bool) (l : List (K × V)) :
    bigOpL and (fun _ kv => Φ kv) (l.filter p) ⊣⊢
    bigOpL and (fun _ kv => if p kv then Φ kv else iprop(True)) l := by
  induction l with
...
theorem filter' {Φ : K → V → PROP} {m : M V} (p : K → V → Bool) :
  ...
  simp only [bigAndM]
  have hperm := LawfulFiniteMap.toList_filter (m := m) (φ := p)
  have heq : bigOpL and (fun _ kv => Φ kv.1 kv.2) (toList (PartialMap.filter p m)) ≡
             bigOpL and (fun _ kv => Φ kv.1 kv.2) ((toList m).filter (fun kv => p kv.1 kv.2)) :=
    BigOpL.perm _ hperm
...
-- variable [FiniteMap M Nat V]
-- variable [FiniteMapLaws M Nat V]
-- variable [FiniteMapSeqLaws M V]

-- /-- Corresponds to `big_andM_map_seq` in Rocq Iris. -/
-- theorem map_seq {Φ : Nat → V → PROP} (start : Nat) (l : List V) :
--     ([∧map] k ↦ x ∈ (FiniteMap.map_seq start l : M), Φ k x) ⊣⊢
--       ([∧list] i ↦ x ∈ l, Φ (start + i) x) := by
--   simp only [bigAndM, bigAndL]
--   have h1 : bigOpL and iprop(True) (fun _ kv => Φ kv.fst kv.snd) (toList (FiniteMap.map_seq start l : M)) ≡
