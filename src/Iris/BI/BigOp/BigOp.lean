...
abbrev bigSepM [BI PROP] {M : Type _ → Type _} {K : Type _} [FiniteMap M K]
    (Φ : K → V → PROP) (m : M V) : PROP :=
  bigOpL sep (fun _ kv => Φ kv.1 kv.2) (toList m)

/-- Big conjunction over a map.
    `bigAndM Φ m` computes `∧_{k ↦ v ∈ m} Φ k v` -/
abbrev bigAndM [BI PROP] {M : Type _ → Type _} {K : Type _} [FiniteMap M K]
    (Φ : K → V → PROP) (m : M V) : PROP :=
  bigOpL and (fun _ kv => Φ kv.1 kv.2) (toList m)
