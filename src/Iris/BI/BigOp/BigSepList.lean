...
theorem emp_l {l : List A} :
  ...
  equiv_iff.mp (BigOpL.const_unit l)
...
theorem sep' {Φ Ψ : Nat → A → PROP} {l : List A} :
  ...
  equiv_iff.mp (BigOpL.op_distrib Φ Ψ l)
...
theorem zip_seq {Φ : A × Nat → PROP} {n : Nat} {l : List A} :
    ([∗list] p ∈ l.zipIdx n, Φ p) ⊣⊢
      [∗list] i ↦ x ∈ l, Φ (x, n + i) :=
  equiv_iff.mp (BigOpL.zipIdx Φ n l)
...
theorem sepL {B : Type _} (Φ : Nat → A → Nat → B → PROP) (l₁ : List A) (l₂ : List B) :
  ...
  | nil =>
    simp only [bigSepL, bigOpL]
    constructor
    · exact Entails.rfl.trans (equiv_iff.mp (BigOpL.const_unit l₂)).2
    · exact (equiv_iff.mp (BigOpL.const_unit l₂)).1
  | cons x xs ih =>
    simp only [bigSepL, bigOpL]
    have ih' := ih (fun i a j b => Φ (i + 1) a j b)
    constructor
    · refine (sep_mono_r ih'.1).trans ?_
      exact equiv_iff.mp (BigOpL.op_distrib _ _ _) |>.2
    · refine equiv_iff.mp (BigOpL.op_distrib _ _ _) |>.1.trans ?_
      exact sep_mono_r ih'.2

/-- Corresponds to `big_sepL_sepM` in Rocq Iris. -/
theorem sepM {V : Type _} {M : Type _ → Type _} {K : Type _} [FiniteMap M K]
    (Φ : Nat → A → K → V → PROP) (l : List A) (m : M V) :
  ...
  calc [∗list] k↦x ∈ l, [∗map] k'↦y ∈ m, Φ k x k' y
      ...
      _ ⊣⊢ [∗list] kv ∈ toList m, [∗list] k↦x ∈ l, Φ k x kv.1 kv.2 :=
          @sepL PROP _ A (K × V) (fun k x _ kv => Φ k x kv.1 kv.2) l (toList m)
      _ ⊣⊢ [∗map] k'↦y ∈ m, [∗list] k↦x ∈ l, Φ k x k' y :=
          equiv_iff.mp <| BigSepL.congr fun _ kv => .rfl

-- /-- Corresponds to `big_sepL_sepS` in Rocq Iris. -/
-- theorem sepS {V : Type _} {S : Type _} [DecidableEq B] [FiniteSet S B] [FiniteSetLaws S B]
