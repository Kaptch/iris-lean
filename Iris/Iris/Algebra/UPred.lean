...
instance : OFE (UPred M) where
  ...
  eq_of_dist {P Q} h := by ext n ⟨x, p⟩; exact h n n x (Nat.le_refl n) p
...
instance [URFunctor F] : COFE.OFunctor (UPredOF F) where
  ...
  map_id z := by
    simp only [uPred_map]; apply UPred.ext; funext n ⟨x, p⟩; simp only; congr 2
    exact URFunctor.map_id x
  map_comp f g f' g' z := by
    simp only [uPred_map]; apply UPred.ext; funext n ⟨x, p⟩; simp only; congr 2
    exact URFunctor.map_comp g' f' g f x
