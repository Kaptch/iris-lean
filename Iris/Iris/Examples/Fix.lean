...
/-!
This example shows that since every OFE is now Leibniz (i.e., `x = y` is definitionally
`x = y`), the fold/unfold isomorphisms of the recursive domain equation solver's fixed point
can be stated as propositional equalities (`=`); see `Dom.unfold_fold` and `Dom.fold_unfold`.
