...
theorem bigAndM_mono {Φ Ψ : K → V → PROP} {m : M V}
  ...
  bigOpM_gen_congr .rfl and_mono (h ·)

@[rocq_alias big_andM_proper]
theorem bigAndM_congr {Φ Ψ : K → V → PROP} {m : M V}
    (h : ∀ {k x}, get? m k = some x → Φ k x = Ψ k x) :
    ([∧map] k ↦ x ∈ m, Φ k x) = [∧map] k ↦ x ∈ m, Ψ k x :=
  bigOpM_congr h

theorem bigAndM_congr_of_forall_eq {Φ Ψ : K → V → PROP} {m : M V}
    (h : ∀ {k x}, Φ k x = Ψ k x) :
    ([∧map] k ↦ x ∈ m, Φ k x) = [∧map] k ↦ x ∈ m, Ψ k x :=
...
theorem bigAndM_and_eq {Φ Ψ : K → V → PROP} {m : M V} :
...
theorem bigAndM_filter {Φ : K → V → PROP} {m : M V} (p : K → V → Bool) :
  ...
  (bigAndM_filter_cond p).trans <| bigOpM_congr fun {k x} _ => by
    match hp : p k x with
