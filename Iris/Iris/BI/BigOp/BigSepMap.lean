...
theorem bigSepM_eq_of_perm {Φ : K → V → PROP} {m₁ m₂ : M V} (h : m₁ ≡ₘ m₂) :
    ([∗map] k ↦ v ∈ m₁, Φ k v) ⊣⊢ ([∗map] k ↦ v ∈ m₂, Φ k v) :=
  equiv_iff.mp (bigOpM_eq_of_perm _ h)

/-- A `bigSepM` over the empty map is `emp`. -/
theorem bigSepM_eq_empty {Φ : K → V → PROP} {m : M V} (h : m = ∅) :
...
theorem bigSepM_singleton {Φ : K → V → PROP} {i : K} {x : V} :
  ...
  equiv_iff.mp <| bigOpM_singleton_eq Φ i x
...
theorem bigSepM_insert {Φ : K → V → PROP} {m : M V} {i : K} {x : V}
  ...
  equiv_iff.mp <| bigOpM_insert_eq Φ x h
...
theorem bigSepM_emp [DecidableEq K] {m : M V} :
  ...
  equiv_iff.mp <| bigOpM_const_unit_eq m
...
@[deprecated "bigSepM_sep_eq.symm" (since := "26/03/30"), rocq_alias big_sepM_sep_2]
...
theorem bigSepM_wand {Φ Ψ : K → V → PROP} {m : M V} :
  ...
  wand_intro <| (equiv_iff.mp bigSepM_sep_eq.symm).1.trans <|
...
theorem bigSepM_impl {Φ Ψ : K → V → PROP} {m : M V} :
  ...
  refine wand_intro <| (sep_mono_right ?_).trans <|
    (equiv_iff.mp bigSepM_sep_eq.symm).1.trans <| bigSepM_mono fun _ => wand_elim_right
...
theorem bigSepM_ofList [DecidableEq K] {Φ : K → V → PROP} {l : List (K × V)}
  ...
  equiv_iff.mp <| bigOpM_ofList_eq Φ l hd
...
theorem bigSepM_later_2 {Φ : K → V → PROP} {m : M V} :
  ...
  bigOpM_gen_congr (R := fun a b => a ⊢ later b)
...
theorem bigSepM_filter [BIAffine PROP] {Φ : K → V → PROP} {m : M V} (p : K → V → Bool) :
  ...
  (bigSepM_filter_cond p).trans <| bigOpM_congr fun {k x} _ => by
    match hp : p k x with
...
theorem bigSepM_union [DecidableEq K] {Φ : K → V → PROP} {m₁ m₂ : M V} (hdisj : m₁ ##ₘ m₂) :
  ...
  equiv_iff.mp <| bigOpM_union_eq Φ m₁ m₂ hdisj

@[rocq_alias big_sepM_subseteq]
theorem bigSepM_subseteq [DecidableEq K] {Φ : K → V → PROP} {m₁ m₂ : M V}
    [∀ k v, Affine (Φ k v)] (h : m₂ ⊆ m₁) :
    ([∗map] k ↦ x ∈ m₁, Φ k x) ⊢ [∗map] k ↦ x ∈ m₂, Φ k x :=
  (equiv_iff.mp <| bigOpM_eq_of_perm Φ <| equiv_iff_eq.mpr <| union_difference_cancel h).2.trans <|
...
theorem bigSepM_lookup_acc_impl [DecidableEq K] {Φ : K → V → PROP} {m : M V} {i : K} {x : V}
  ...
  case R2 =>
    refine (equiv_iff.mp bigSepM_sep_eq.symm).1.trans ?_
...
theorem bigSepM_sep_zipWith {A B C : Type _}
  ...
  equiv_iff.mp <| bigOpM_sep_zipWith_eq Φ₁ Φ₂ hg₁ hg₂ hdom
...
theorem bigSepM_sep_zip {A B : Type _}
    ...
    ([∗map] k ↦ xy ∈ zip m₁ m₂, Φ₁ k xy.1 ∗ Φ₂ k xy.2) ⊣⊢
      ([∗map] k ↦ x ∈ m₁, Φ₁ k x) ∗ [∗map] k ↦ y ∈ m₂, Φ₂ k y :=
  equiv_iff.mp <| bigOpM_sep_zip_eq Φ₁ Φ₂ hdom
...
theorem bigSepM_impl_strong [DecidableEq K] {M₂ : Type _ → Type _} {V₂ : Type _}
  ...
  case hemp =>
    ...
    refine (equiv_iff.mp <| bigOpM_eq_of_perm Φ fun k => ?_).2
  ...
  case hind =>
    ...
    | none =>
      ...
      refine sep_mono_right (equiv_iff.mp <| bigOpM_eq_of_perm Φ fun k => ?_).2
    ...
    | some x =>
      ...
      refine sep_mono_right <| sep_mono_right (equiv_iff.mp <| bigOpM_eq_of_perm Φ fun k => ?_).2
